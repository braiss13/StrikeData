...
{
    /*
        Imports TEAM-level FIELDING statistics from TeamRankings and saves them into StatType/TeamStat under the "Fielding" category.
    ...
    */
    public class FieldingImporter
    {
        ...
        // Entry point for Fielding: iterates over TeamRankings fielding endpoints  and imports each metric's per-game values for all teams.
        ...
        */
        public async Task ImportHittingTeamStatsTR(string statTypeName, string url)
        {
            ...
            // Skip the header row: only iterate data rows (one per team).
            ...
            foreach (var row in rows)
            {
                ...
                // Guard against unexpected table shapes. expect at least 8 cells.
